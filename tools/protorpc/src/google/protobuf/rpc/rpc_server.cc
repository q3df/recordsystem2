...
void Server::AddService(Service* service, bool ownership) {
  ...
  if(it != service_map_.end()) {
    GOOGLE_LOG(FATAL) << "AddService: Service already exist, " << name;
  }
...
const ::google::protobuf::rpc::Error Server::CallMethod(
  ...
  if(method == NULL) {
    return Error::New("rpc: can't find method " + method_name);
  }
  auto service = findService(method);
  if(service == NULL) {
    return Error::New("rpc: can't find service " + method_name);
  }
...
const ::google::protobuf::rpc::Error Server::CallMethod(
  ...
  auto service = findService(method);
  if(service == NULL) {
        return Error::New("rpc: can't find service " + Service::GetServiceMethodName(method));
  }
  return service->CallMethod(method, request, response);
}
