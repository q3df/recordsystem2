...
int Q3Vm::SysCalls(byte *memoryBase, int cmd, int *args) {
	...
	case G_PRINT: // ( const char *string );
		syscall_->Printf((const char *)ptr_q3df(0));
		return 0;
	case G_ERROR: // ( const char *string );
		syscall_->Error((const char *)ptr_q3df(0));
		return 0;
	...
	case G_CVAR_REGISTER: // ( vmCvar_t *vmCvar, const char *varName, const char *defaultValue, int flags );
		syscall_->CvarRegister((vmCvar_t *)ptr_q3df(0), (const char *)ptr_q3df(1), (const char*)ptr_q3df(2), arg_q3df(3));
		return 0;
	case G_CVAR_UPDATE:	// ( vmCvar_t *vmCvar );
		syscall_->CvarUpdate((vmCvar_t *)ptr_q3df(0));
		return 0;
	case G_CVAR_SET: // ( const char *var_name, const char *value );
		syscall_->CvarSet((const char *)ptr_q3df(0), (const char *)ptr_q3df(1));
		return 0;
	case G_CVAR_VARIABLE_INTEGER_VALUE:	// ( const char *var_name );
		return syscall_->CvarVariableIntegerValue((const char *)ptr_q3df(0));
	case G_CVAR_VARIABLE_STRING_BUFFER:	// ( const char *var_name, char *buffer, int bufsize );
		syscall_->CvarVariableStringBuffer((const char *)ptr_q3df(0), (char *)ptr_q3df(1), arg_q3df(2));
		return 0;
	case G_ARGC: // ( void );
		return syscall_->Argc();
	case G_ARGV: // ( int n, char *buffer, int bufferLength );
		syscall_->Argv(arg_q3df(0), (char *)ptr_q3df(1), arg_q3df(2));
		return 0;
	case G_FS_FOPEN_FILE: // ( const char *qpath, fileHandle_t *file, fsMode_t mode );
		return syscall_->FSFOpenFile((const char *)ptr_q3df(0), (fileHandle_t *)ptr_q3df(1), (fsMode_t)arg_q3df(2));
	case G_FS_READ: // ( void *buffer, int len, fileHandle_t f );
		syscall_->FSRead(ptr_q3df(0), arg_q3df(1), arg_q3df(2));
		return 0;
	case G_FS_WRITE: // ( const void *buffer, int len, fileHandle_t f );
		syscall_->FSWrite(ptr_q3df(0), arg_q3df(1), arg_q3df(2));
		return 0;
	case G_FS_FCLOSE_FILE: // ( fileHandle_t f );
		syscall_->FSFCloseFile(arg_q3df(0));
		return 0;
	case G_SEND_CONSOLE_COMMAND: // ( int exec_when, const char *text )
		syscall_->SendConsoleCommand(arg_q3df(0), (const char *)ptr_q3df(1));
		return 0;
	case G_LOCATE_GAME_DATA: // ( gentity_t *gEnts, int numGEntities, int sizeofGEntity_t, playerState_t *clients, int sizeofGameClient );
		syscall_->LocateGameData((gentity_t *)ptr_q3df(0), arg_q3df(1), arg_q3df(2), (playerState_t*)ptr_q3df(3), arg_q3df(4));
		return 0;
	case G_DROP_CLIENT: // ( int clientNum, const char *reason );
		syscall_->DropClient(arg_q3df(0), (const char *)ptr_q3df(1));
		return 0;
	case G_SEND_SERVER_COMMAND:	// ( int clientNum, const char *fmt );
		syscall_->SendServerCommand(arg_q3df(0), (const char *)ptr_q3df(1));
		return 0;
	case G_SET_CONFIGSTRING: // ( int num, const char *string );
		syscall_->SetConfigstring(arg_q3df(0), (const char *)ptr_q3df(1));
		return 0;
	case G_GET_CONFIGSTRING: // ( int num, char *buffer, int bufferSize );
		syscall_->GetConfigstring(arg_q3df(0), (char *)ptr_q3df(1), arg_q3df(2));
		return 0;
	case G_GET_USERINFO: // ( int num, char *buffer, int bufferSize );
		syscall_->GetUserinfo(arg_q3df(0), (char *)ptr_q3df(1), arg_q3df(2));
		return 0;
	case G_SET_USERINFO: // ( int num, const char *buffer );
		syscall_->SetUserinfo(arg_q3df(0), (const char *)ptr_q3df(1));
		return 0;
	case G_GET_SERVERINFO: // ( char *buffer, int bufferSize );
		syscall_->GetServerinfo((char *)ptr_q3df(0), arg_q3df(1));
		return 0;
	case G_SET_BRUSH_MODEL:	// ( gentity_t *ent, const char *name );
		syscall_->SetBrushModel((gentity_t *)ptr_q3df(0), (const char *)ptr_q3df(1));
		return 0;
	case G_TRACE: // ( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask );
		syscall_->Trace((trace_t *)ptr_q3df(0), (const vec_t *)ptr_q3df(1), (const vec_t *)ptr_q3df(2), (const vec_t *)ptr_q3df(3), (const vec_t *)ptr_q3df(4), arg_q3df(5), arg_q3df(6));
		return 0;
	case G_POINT_CONTENTS: // ( const vec3_t point, int passEntityNum );
		return syscall_->PointContents((const vec_t *)ptr_q3df(0), arg_q3df(1));
	case G_IN_PVS: // ( const vec3_t p1, const vec3_t p2 );
		return syscall_->InPVS((const vec_t *)ptr_q3df(0), (const vec_t *)ptr_q3df(1));
	case G_IN_PVS_IGNORE_PORTALS: // ( const vec3_t p1, const vec3_t p2 );
		return syscall_->InPVSIgnorePortals((const vec_t *)ptr_q3df(0), (const vec_t *)ptr_q3df(1));
	case G_ADJUST_AREA_PORTAL_STATE: // ( gentity_t *ent, qboolean open );
		syscall_->AdjustAreaPortalState((gentity_t *)ptr_q3df(0), (qboolean)arg_q3df(1));
		return 0;
	case G_AREAS_CONNECTED: // ( int area1, int area2 );
		return syscall_->AreasConnected(arg_q3df(0), arg_q3df(1));
	case G_LINKENTITY: // ( gentity_t *ent );
		syscall_->LinkEntity((gentity_t *)ptr_q3df(0));
		return 0;
	case G_UNLINKENTITY: // ( gentity_t *ent );
		syscall_->UnlinkEntity((gentity_t *)ptr_q3df(0));
		return 0;
	case G_ENTITIES_IN_BOX: // ( const vec3_t mins, const vec3_t maxs, gentity_t **list, int maxcount );
		return syscall_->EntitiesInBox((const vec_t *)ptr_q3df(0), (const vec_t *)ptr_q3df(1), (gentity_t **)ptr_q3df(2), arg_q3df(3));
	case G_ENTITY_CONTACT: // ( const vec3_t mins, const vec3_t maxs, const gentity_t *ent );
		return syscall_->EntityContact((const vec_t *)ptr_q3df(0), (const vec_t *)ptr_q3df(1), (const gentity_t *)ptr_q3df(2));
	case G_BOT_ALLOCATE_CLIENT:		// ( void );
		return syscall_->BotAllocateClient();
	case G_BOT_FREE_CLIENT:	// ( int clientNum );
		syscall_->BotFreeClient(arg_q3df(0));
		return 0;
	case G_GET_USERCMD: // ( int clientNum, usercmd_t *cmd )
		syscall_->GetUsercmd(arg_q3df(0), (usercmd_t *)ptr_q3df(1));
		return 0;
	case G_GET_ENTITY_TOKEN: // qboolean ( char *buffer, int bufferSize )
		return syscall_->GetEntityToken((char *)ptr_q3df(0), arg_q3df(1));
	case G_FS_GETFILELIST: // ( const char *path, const char *extension, char *listbuf, int bufsize ) {
		return syscall_->FSGetFileList((const char *)ptr_q3df(0), (const char *)ptr_q3df(1), (char *)ptr_q3df(2), arg_q3df(3));
	case G_DEBUG_POLYGON_CREATE: //(int color, int numPoints, vec3_t *points)
		return syscall_->DebugPolygonCreate(arg_q3df(0), arg_q3df(1), (vec3_t *)ptr_q3df(2));
	case G_DEBUG_POLYGON_DELETE: //(int id)
		syscall_->DebugPolygonDelete(arg_q3df(0));
		return 0;
	case G_REAL_TIME: //( qtime_t *qtime )
		return syscall_->RealTime((qtime_t *)ptr_q3df(0));
	case G_SNAPVECTOR: // (float *v)
		syscall_->SnapVector((float *)ptr_q3df(0));
		return 0;
	case G_TRACECAPSULE: // ( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask );
		syscall_->TraceCapsule((trace_t *)ptr_q3df(0), (const vec_t *)ptr_q3df(1), (const vec_t *)ptr_q3df(2), (const vec_t *)ptr_q3df(3), (const vec_t *)ptr_q3df(4), arg_q3df(5), arg_q3df(6));
		return 0;
	case G_ENTITY_CONTACTCAPSULE: // ( const vec3_t mins, const vec3_t maxs, const gentity_t *ent );
		return syscall_->EntityContactCapsule((const vec_t *)ptr_q3df(0), (const vec_t *)ptr_q3df(1), (const gentity_t *)ptr_q3df(2));
	case G_FS_SEEK: //( fileHandle_t f, long offset, int origin )
		return syscall_->FSSeek(arg_q3df(0), arg_q3df(1), arg_q3df(2));
	...
	case BOTLIB_LIBVAR_SET:	//(char *var_name, char *value)
		return syscall_->BotLibVarSet((char *)ptr_q3df(0), (char *)ptr_q3df(1));
	case BOTLIB_LIBVAR_GET:	//(char *var_name, char *value, int size)
		return syscall_->BotLibVarGet((char *)ptr_q3df(0), (char *)ptr_q3df(1), arg_q3df(2));
	case BOTLIB_PC_ADD_GLOBAL_DEFINE: //(char* string)
		return syscall_->BotLibDefine((char *)ptr_q3df(0));
	case BOTLIB_START_FRAME: //(float time)
		return syscall_->BotLibStartFrame(VMF(0));
	case BOTLIB_LOAD_MAP: //(const char *mapname)
		return syscall_->BotLibLoadMap((const char *)ptr_q3df(0));
	case BOTLIB_UPDATENTITY: //(int ent, void /* struct bot_updateentity_s */ *bue)
		return syscall_->BotLibUpdateEntity(arg_q3df(0), ptr_q3df(1));
	case BOTLIB_TEST: //(int parm0, char *parm1, vec3_t parm2, vec3_t parm3)
		return syscall_->BotLibTest(arg_q3df(0), (char *)ptr_q3df(1), (vec_t *)ptr_q3df(2), (vec_t *)ptr_q3df(3));
	case BOTLIB_GET_SNAPSHOT_ENTITY: //( int clientNum, int sequence )
		return syscall_->BotGetSnapshotEntity(arg_q3df(0), arg_q3df(1));
	case BOTLIB_GET_CONSOLE_MESSAGE: //(int clientNum, char *message, int size)
		return syscall_->BotGetServerCommand(arg_q3df(0), (char *)ptr_q3df(1), arg_q3df(2));
	case BOTLIB_USER_COMMAND: //(int clientNum, usercmd_t *ucmd)
		syscall_->BotUserCommand(arg_q3df(0), (usercmd_t *)ptr_q3df(1));
		return 0;
	case BOTLIB_AAS_ENTITY_INFO: //(int entnum, void /* struct aas_entityinfo_s */ *info)
		syscall_->AASEntityInfo(arg_q3df(0), ptr_q3df(1));
		return 0;
	...
	case BOTLIB_AAS_PRESENCE_TYPE_BOUNDING_BOX:	//(int presencetype, vec3_t mins, vec3_t maxs)
		syscall_->AASPresenceTypeBoundingBox(arg_q3df(0), (vec_t *)ptr_q3df(1), (vec_t *)ptr_q3df(2));
		return 0;
	case BOTLIB_AAS_TIME: //(void)
		return PASSFLOAT2(syscall_->AASTime());
	case BOTLIB_AAS_POINT_AREA_NUM:	//(vec3_t point)
		return syscall_->AASPointAreaNum((vec_t *)ptr_q3df(0));
	case BOTLIB_AAS_POINT_REACHABILITY_AREA_INDEX:	//(vec3_t point)
		return syscall_->AASPointReachabilityAreaIndex((vec_t *)ptr_q3df(0));
	case BOTLIB_AAS_TRACE_AREAS: //(vec3_t start, vec3_t end, int *areas, vec3_t *points, int maxareas)
		return syscall_->AASTraceAreas((vec_t *)ptr_q3df(0), (vec_t *)ptr_q3df(1), (int *)ptr_q3df(2), (vec3_t *)ptr_q3df(3), arg_q3df(4));
	case BOTLIB_AAS_BBOX_AREAS:	//(vec3_t absmins, vec3_t absmaxs, int *areas, int maxareas)
		return syscall_->AASBBoxAreas((vec_t *)ptr_q3df(0), (vec_t *)ptr_q3df(1), (int *)ptr_q3df(2), arg_q3df(3));
	case BOTLIB_AAS_AREA_INFO: //( int areanum, void /* struct aas_areainfo_s */ *info )
		return syscall_->AASAreaInfo(arg_q3df(0), ptr_q3df(1));
	case BOTLIB_AAS_POINT_CONTENTS:	//(vec3_t point)
		return syscall_->AASPointContents((vec_t *)ptr_q3df(0));
	case BOTLIB_AAS_NEXT_BSP_ENTITY: //(int ent)
		return syscall_->AASNextBSPEntity(arg_q3df(0));
	case BOTLIB_AAS_VALUE_FOR_BSP_EPAIR_KEY: //(int ent, char *key, char *value, int size)
		return syscall_->AASValueForBSPEpairKey(arg_q3df(0), (char *)ptr_q3df(1), (char *)ptr_q3df(2), arg_q3df(3));
	case BOTLIB_AAS_VECTOR_FOR_BSP_EPAIR_KEY: //(int ent, char *key, vec3_t v)
		return syscall_->AASVectorForBSPEpairKey(arg_q3df(0), (char *)ptr_q3df(1), (vec_t  *)ptr_q3df(2));
	case BOTLIB_AAS_FLOAT_FOR_BSP_EPAIR_KEY: //(int ent, char *key, float *value)
		return syscall_->AASFloatForBSPEpairKey(arg_q3df(0), (char *)ptr_q3df(1), (float *)ptr_q3df(2));
	case BOTLIB_AAS_INT_FOR_BSP_EPAIR_KEY: //(int ent, char *key, int *value)
		return syscall_->AASIntForBSPEpairKey(arg_q3df(0), (char *)ptr_q3df(1), (int *)ptr_q3df(2));
	case BOTLIB_AAS_AREA_REACHABILITY: //(int areanum)
		return syscall_->AASAreaReachability(arg_q3df(0));
	case BOTLIB_AAS_AREA_TRAVEL_TIME_TO_GOAL_AREA: //(int areanum, vec3_t origin, int goalareanum, int travelflags)
		return syscall_->AASAreaTravelTimeToGoalArea(arg_q3df(0), (vec_t *)ptr_q3df(1), arg_q3df(2), arg_q3df(3));
	case BOTLIB_AAS_ENABLE_ROUTING_AREA: //(int areanum, int enable)
		return syscall_->AASEnableRoutingArea(arg_q3df(0), arg_q3df(1));
	case BOTLIB_AAS_PREDICT_ROUTE: //(void /*struct aas_predictroute_s*/ *route, int areanum, vec3_t origin, int goalareanum, int travelflags, int maxareas, int maxtime, int stopevent, int stopcontents, int stoptfl, int stopareanum)
		return syscall_->AASPredictRoute(ptr_q3df(0), arg_q3df(1), (vec_t *)ptr_q3df(2), arg_q3df(3), arg_q3df(4), arg_q3df(5), arg_q3df(6), arg_q3df(7), arg_q3df(8), arg_q3df(9), arg_q3df(10));
	case BOTLIB_AAS_ALTERNATIVE_ROUTE_GOAL:	//(vec3_t start, int startareanum, vec3_t goal, int goalareanum, int travelflags, void /*struct aas_altroutegoal_s*/ *altroutegoals, int maxaltroutegoals, int type)
		return syscall_->AASAlternativeRouteGoals((vec_t *)ptr_q3df(0), arg_q3df(1), (vec_t *)ptr_q3df(2), arg_q3df(3), arg_q3df(4), ptr_q3df(5), arg_q3df(6), arg_q3df(7));
	case BOTLIB_AAS_SWIMMING: //(vec3_t origin)
		return syscall_->AASSwimming((vec_t *)ptr_q3df(0));
	...
	case BOTLIB_AI_INITIAL_CHAT: //(int chatstate, char *type, int mcontext, char *var0, char *var1, char *var2, char *var3, char *var4, char *var5, char *var6, char *var7 )
		syscall_->BotInitialChat(arg_q3df(0), (char *)ptr_q3df(1), arg_q3df(2), (char *)ptr_q3df(3), (char *)ptr_q3df(4), (char *)ptr_q3df(5), (char *)ptr_q3df(6), (char *)ptr_q3df(7), (char *)ptr_q3df(8), (char *)ptr_q3df(9), (char *)ptr_q3df(10));
		return 0;
	case BOTLIB_AI_NUM_INITIAL_CHATS: //(int chatstate, char *type)
		return syscall_->BotNumInitialChats(arg_q3df(0), (char *)ptr_q3df(1));
	...
	case BOTLIB_AI_CHOOSE_NBG_ITEM:	//(int goalstate, vec3_t origin, int *inventory, int travelflags, void /* struct bot_goal_s */ *ltg, float maxtime)
		return syscall_->BotChooseNBGItem(arg_q3df(0), (vec_t *)ptr_q3df(1), (int *)ptr_q3df(2), arg_q3df(3), ptr_q3df(4), VMF(5));
	case BOTLIB_AI_TOUCHING_GOAL: //(vec3_t origin, void /* struct bot_goal_s */ *goal)
		return syscall_->BotTouchingGoal((vec_t *)ptr_q3df(0), ptr_q3df(1));
	case BOTLIB_AI_ITEM_GOAL_IN_VIS_BUT_NOT_VISIBLE: //(int viewer, vec3_t eye, vec3_t viewangles, void /* struct bot_goal_s */ *goal)
		return syscall_->BotItemGoalInVisButNotVisible(arg_q3df(0), (vec_t *)ptr_q3df(1), (vec_t *)ptr_q3df(2), ptr_q3df(3));
	case BOTLIB_AI_GET_LEVEL_ITEM_GOAL:	//(int index, char *classname, void /* struct bot_goal_s */ *goal)
		return syscall_->BotGetLevelItemGoal(arg_q3df(0), (char *)ptr_q3df(1), ptr_q3df(2));
	case BOTLIB_AI_GET_NEXT_CAMP_SPOT_GOAL:	//(int num, void /* struct bot_goal_s */ *goal)
		return syscall_->BotGetNextCampSpotGoal(arg_q3df(0), ptr_q3df(1));
	case BOTLIB_AI_GET_MAP_LOCATION_GOAL: //(char *name, void /* struct bot_goal_s */ *goal)
		return syscall_->BotGetMapLocationGoal((char *)ptr_q3df(0), ptr_q3df(1));
	case BOTLIB_AI_AVOID_GOAL_TIME: //(int goalstate, int number)
		return PASSFLOAT2(syscall_->BotAvoidGoalTime(arg_q3df(0), arg_q3df(1)));
	case BOTLIB_AI_SET_AVOID_GOAL_TIME:	//(int goalstate, int number, float avoidtime)
		syscall_->BotSetAvoidGoalTime(arg_q3df(0), arg_q3df(1), VMF(2));
		return 0;
	...
	case BOTLIB_AI_LOAD_ITEM_WEIGHTS: //(int, char*)
		return syscall_->BotLoadItemWeights(arg_q3df(0), (char *)ptr_q3df(1));
	case BOTLIB_AI_FREE_ITEM_WEIGHTS: //(int goalstate)
		syscall_->BotFreeItemWeights(arg_q3df(0));
		return 0;
	case BOTLIB_AI_INTERBREED_GOAL_FUZZY_LOGIC:	//(int,int,int)
		syscall_->BotInterbreedGoalFuzzyLogic(arg_q3df(0), arg_q3df(1), arg_q3df(2));
		return 0;
	case BOTLIB_AI_SAVE_GOAL_FUZZY_LOGIC: //(int, char*)
		syscall_->BotSaveGoalFuzzyLogic(arg_q3df(0), (char *)ptr_q3df(1));
		return 0;
	case BOTLIB_AI_MUTATE_GOAL_FUZZY_LOGIC: //(int goalstate, float range)
		syscall_->BotMutateGoalFuzzyLogic(arg_q3df(0), VMF(1));
		return 0;
	case BOTLIB_AI_ALLOC_GOAL_STATE: //(int state)
		return syscall_->BotAllocGoalState(arg_q3df(0));
	case BOTLIB_AI_FREE_GOAL_STATE: //(int handle)
		syscall_->BotFreeGoalState(arg_q3df(0));
		return 0;
	case BOTLIB_AI_RESET_MOVE_STATE: //(int movestate)
		syscall_->BotResetMoveState(arg_q3df(0));
		return 0;
	case BOTLIB_AI_ADD_AVOID_SPOT: //(int movestate, vec3_t origin, float radius, int type)
		syscall_->BotAddAvoidSpot(arg_q3df(0), (vec_t *)ptr_q3df(1), VMF(2), arg_q3df(3));
		return 0;
	case BOTLIB_AI_MOVE_TO_GOAL: //(void /* struct bot_moveresult_s */ *result, int movestate, void /* struct bot_goal_s */ *goal, int travelflags)
		syscall_->BotMoveToGoal(ptr_q3df(0), arg_q3df(1), ptr_q3df(2), arg_q3df(3));
		return 0;
	case BOTLIB_AI_MOVE_IN_DIRECTION: //(int movestate, vec3_t dir, float speed, int type)
		return syscall_->BotMoveInDirection(arg_q3df(0), (vec_t *)ptr_q3df(1), VMF(2), arg_q3df(3));
	case BOTLIB_AI_RESET_AVOID_REACH: //(int movestate)
		syscall_->BotResetAvoidReach(arg_q3df(0));
		return 0;
	case BOTLIB_AI_RESET_LAST_AVOID_REACH: //(int movestate)
		syscall_->BotResetLastAvoidReach(arg_q3df(0));
		return 0;
	case BOTLIB_AI_REACHABILITY_AREA: //(vec3_t origin, int testground)
		return syscall_->BotReachabilityArea((vec_t *)ptr_q3df(0), arg_q3df(1));
	case BOTLIB_AI_MOVEMENT_VIEW_TARGET: //(int movestate, void /* struct bot_goal_s */ *goal, int travelflags, float lookahead, vec3_t target)
		return syscall_->BotMovementViewTarget(arg_q3df(0), ptr_q3df(1), arg_q3df(2), VMF(3), (vec_t *)ptr_q3df(4));
	case BOTLIB_AI_PREDICT_VISIBLE_POSITION: //(vec3_t origin, int areanum, void /* struct bot_goal_s */ *goal, int travelflags, vec3_t target)
		return syscall_->BotPredictVisiblePosition((vec_t *)ptr_q3df(0), arg_q3df(1), ptr_q3df(2), arg_q3df(3), (vec_t *)ptr_q3df(4));
	case BOTLIB_AI_ALLOC_MOVE_STATE: //(void)
		return syscall_->BotAllocMoveState();
	case BOTLIB_AI_FREE_MOVE_STATE:	//(int handle)
		syscall_->BotFreeMoveState(arg_q3df(0));
		return 0;
	case BOTLIB_AI_INIT_MOVE_STATE:	//(int handle, void* initmove)
		syscall_->BotInitMoveState(arg_q3df(0), ptr_q3df(1));
		return 0;
	case BOTLIB_AI_CHOOSE_BEST_FIGHT_WEAPON: //(int weaponstate, int *inventory)
		return syscall_->BotChooseBestFightWeapon(arg_q3df(0), (int *)ptr_q3df(1));
	case BOTLIB_AI_GET_WEAPON_INFO: //(int weaponstate, int weapon, void /* struct weaponinfo_s */ *weaponinfo)
		syscall_->BotGetWeaponInfo(arg_q3df(0), arg_q3df(1), ptr_q3df(2));
		return 0;
	case BOTLIB_AI_LOAD_WEAPON_WEIGHTS:	//(int, char*)
		return syscall_->BotLoadWeaponWeights(arg_q3df(0), (char *)ptr_q3df(1));
	case BOTLIB_AI_ALLOC_WEAPON_STATE: //(void)
		return syscall_->BotAllocWeaponState();
	case BOTLIB_AI_FREE_WEAPON_STATE: //(int)
		syscall_->BotFreeWeaponState(arg_q3df(0));
		return 0;
	case BOTLIB_AI_RESET_WEAPON_STATE: //(int)
		syscall_->BotResetWeaponState(arg_q3df(0));
		return 0;
	case BOTLIB_AI_GENETIC_PARENTS_AND_CHILD_SELECTION:	//(int numranks, float *ranks, int *parent1, int *parent2, int *child)
		return syscall_->GeneticParentsAndChildSelection(arg_q3df(0), (float *)ptr_q3df(1), (int *)ptr_q3df(2), (int *)ptr_q3df(3), (int *)ptr_q3df(4));
	case BOTLIB_PC_LOAD_SOURCE: //(const char*)
		return syscall_->PCLoadSource((const char *)ptr_q3df(0));
	case BOTLIB_PC_FREE_SOURCE:	//(int)
		return syscall_->PCFreeSource(arg_q3df(0));
	case BOTLIB_PC_READ_TOKEN: //(int, void*)
		return syscall_->PCReadToken(arg_q3df(0), (pc_token_t *)ptr_q3df(1));
	case BOTLIB_PC_SOURCE_FILE_AND_LINE: //( int handle, char *filename, int *line )
		return syscall_->PCSourceFileAndLine(arg_q3df(0), (char *)ptr_q3df(1), (int *)ptr_q3df(2));
	case G_MEMSET: //(void* dest, int c, size_t count)
		syscall_->MemSet(ptr_q3df(0), arg_q3df(1), arg_q3df(2));
		return 0;
	case G_MEMCPY: //(void *dest, const void *src, size_t count)
		syscall_->MemCpy(ptr_q3df(0), ptr_q3df(1), arg_q3df(2));
		return 0;
	case G_STRNCPY: //(char *strDest, const char *strSource, size_t count )
		return (int)syscall_->StringNCopy((char *)ptr_q3df(0), (const char *)ptr_q3df(1), arg_q3df(2));
	case G_SIN:	//(float)
		return PASSFLOAT2(syscall_->Sin(VMF(0)));
		//return syscall_->syscall_(G_SIN, arg_q3df(0));
	case G_COS:	//(float)
		return PASSFLOAT2(syscall_->Cos(VMF(0)));
		//return syscall_->syscall_(G_COS, arg_q3df(0));
	case G_ATAN2: //(float, float)
		return PASSFLOAT2(syscall_->ATan2(VMF(0), VMF(1)));
		//return syscall_->syscall_(G_ATAN2, arg_q3df(0));
	case G_SQRT: //(float)
		return PASSFLOAT2(syscall_->Sqrt(VMF(0)));
		//return syscall_->syscall_(G_SQRT, arg_q3df(0));
	case G_FLOOR: //(float)
		return PASSFLOAT2(syscall_->Floor(VMF(0)));
		//return syscall_->syscall_(G_FLOOR, arg_q3df(0));
	case G_CEIL: //(float)
		return PASSFLOAT2(syscall_->Ceil(VMF(0)));
		//return syscall_->syscall_(G_CEIL, arg_q3df(0));
